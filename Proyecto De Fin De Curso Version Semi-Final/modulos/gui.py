...
def q():
    ...
    select = textob.curselection()
    if select:
        song = textob.get(select)
        play_song(artista, song)
...
def f():
    ...
    if cancionSelec:
        songSelec = textoR.get(cancionSelec)
        anadir_a_fav(lSong, songSelec)

# Funcion para llamar a otra
...
Button(frame_b, text='Reproducir', bg='#1DB954', fg='white', font=('Arial', 14), command=q).pack(pady=10)
